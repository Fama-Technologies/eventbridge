...
export default function DualMonthCalendar({ 
        ...
        <div className="grid grid-cols-7 gap-0.5 mb-1">
          {weekDays.map((day, idx) => (
            <div 
              ...
              className="w-5 h-5 text-center text-[10px] text-white/50 flex items-center justify-center"
            >
              {day}
            </div>
          ))}
            ...
            return (
              <button
                ...
                className={`w-5 h-5 text-[11px] flex items-center justify-center rounded transition-colors
                  ${isPast ? 'text-white/20 cursor-not-allowed' : 'text-white/70 hover:bg-white/10'}
  ...
  return (
    <div className="fixed inset-0 z-50" onClick={onClose}>
      <div 
        className="fixed bg-[#1E1E1E] rounded-2xl shadow-2xl p-5 border border-white/10"
        style={{ 
          top: position.top, 
          left: position.left,
